"""Recrear contenedor web tras error ContainerConfig de docker-compose v1."""
import os
import sys
...
def main() -> None:
    ...
    project = "/opt/wowed"
    cmds = [
        f"cd {project} && docker-compose stop web || true",
        f"cd {project} && docker-compose rm -f web || true",
        "docker rm -f wowed_web_1 2>/dev/null || true",
        f"cd {project} && docker-compose up -d --build web",
